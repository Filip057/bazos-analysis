"""
Power Resolution System

This module handles comparison and resolution of power values extracted
...
class PowerResolution:
    """
    Complete power resolution result with metadata.
...
class PowerNormalizer:
    """Handles normalization of power values for comparison."""
    ...
    def normalize(raw_value: Optional[str]) -> Optional[str]:
        """
        Normalize power value to standard format: "{number}{unit}"
        ...
        match = re.match(r'(\d+)\s*([a-zá-ží]+)', cleaned)
        if not match:
            return None
        ...
        # Normalize unit
        normalized_unit = PowerNormalizer.UNITS.get(unit)
...
class PowerResolver:
    """
    Resolves disagreements between ML and regex power extractions.

    Resolution strategy:
    1. If both agree (after normalization) -> HIGH confidence
    2. If minor formatting difference -> AUTO_NORMALIZED, HIGH confidence
    3. If major disagreement -> Use regex (more conservative), MEDIUM confidence
    4. If only one source available -> Use it, MEDIUM confidence
    5. If neither available -> None, LOW confidence
    """

    def __init__(self, prefer_ml: bool = False):
        """
        Initialize resolver.

        Args:
            prefer_ml: If True, prefer ML in case of disagreement (default: False, prefer regex)
        """
    ...
    def resolve(
        ...
        """
        Resolve power value from ML and regex extractions.

        Args:
            ml_raw: Raw power value from ML extraction
            regex_raw: Raw power value from regex extraction
            manual_override: Optional manual value (takes precedence)

        Returns:
            PowerResolution object with complete resolution metadata
        """
    ...
    def _classify_disagreement(
        ...
        if normalized_ml is not None and normalized_regex is not None:
            ...
            # Check if only formatting differs (already normalized, so this means different values)
...
def resolve_power(
    ml_raw: Optional[str],
    regex_raw: Optional[str],
    prefer_ml: bool = False,
    manual_override: Optional[str] = None
) -> PowerResolution:
    """
    Convenience function to resolve power values.

    Args:
        ml_raw: Raw power value from ML extraction
        regex_raw: Raw power value from regex extraction
        prefer_ml: If True, prefer ML in case of disagreement
        manual_override: Optional manual value (takes precedence)
    ...
    Example:
        ...
        0.95
    """
    resolver = PowerResolver(prefer_ml=prefer_ml)
